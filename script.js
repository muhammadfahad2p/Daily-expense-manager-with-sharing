 ...
 document.addEventListener('DOMContentLoaded', () => {
     ...
     setTimeout(checkGoogleLoaded, 300);

 });

function disconnectDrive() {
    localStorage.removeItem(DRIVE_LOGIN_KEY);
 ...
 function triggerCopy() {
     try {
         document.getElementById('copyDateInput').showPicker()
     } catch (e) {
         document.getElementById('copyDateInput').click()
     }
 }
...
function updateDriveUI() {
    ...
    const isLoggedIn = localStorage.getItem(DRIVE_LOGIN_KEY) === '1';
 ...
 async function backupToDrive() {
     try {
         ...
         const q = "name = 'expense_manager_backup.json' and trashed = false";
 ...
 function executeExport(type) {
     ...
     const file = new File([csv], "expenses.csv", {
         type: 'text/csv'
     });
     if (type === 'share' && navigator.canShare && navigator.canShare({
             files: [file]
         })) navigator.share({
         files: [file]
     });
